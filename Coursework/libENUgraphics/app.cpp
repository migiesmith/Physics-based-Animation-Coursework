...
namespace graphics_framework
{
	...
	void app::run()
	{
		...
		while (renderer::is_running())
		{
			...
			if (!_update_func._Empty())
			{
				// Call update
				if (!_update_func(seconds))
				{
					// Log update exit
					std::clog << "LOG - updated returned false.  Exiting" << std::endl;
					break;
				}
			}
